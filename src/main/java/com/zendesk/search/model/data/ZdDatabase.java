 ...
 * Class to hold the data from the json files(user, organization, tickets)
 * Also contains auxilary helper methods and index maps to perform search in a speedier manner
 * This is helpful for large datasets
 */
@Getter
public final class ZdDatabase implements ZdData {
    ...
    private static List<Ticket> createTicketList() {
        ...
        List<Ticket> tickets;
        try {
            tickets = objectMapper.readValue(ResourceUtils.getFile("classpath:tickets.json"),
    ...
    private static List<User> createUserList() {
        ...
        List<User> users;
        try {
            users = objectMapper.readValue(ResourceUtils.getFile("classpath:users.json"),
    ...
    private static Map<String, Organization> createOrgIndex(List<Organization> organizations) {
        ...
        return ImmutableMap.copyOf(organizations.stream()
    ...
    private static Map<String, User> createUserIndex(List<User> users) {
        ...
        return ImmutableMap.copyOf(users.stream()
    ...
    private static Map<String, Ticket> createTicketIndex(List<Ticket> tickets) {
        ...
        return ImmutableMap.copyOf(tickets.stream()
    ...
    private List<Organization> createOrgList() {
        ...
        try {
            organizations = objectMapper.readValue(ResourceUtils.getFile("classpath:organizations.json"),
                    objectMapper.getTypeFactory().constructCollectionType(ArrayList.class, Organization.class));
        } catch (IOException e) {
    ...
    /**
     * Creates a map of json properties in data class to the corresponding java fields
     * This is used in mapping the user input from console to the corresponding java field
     *
     * @param clazz
     * @return
     */
    private Map<String, String> jsonPropToJavaField(Class clazz) {
        return Arrays.stream(clazz.getDeclaredFields())
                ...
                .collect(Collectors.toMap(p ->
                                p.getAnnotation(JsonProperty.class).value(),
                        Field::getName));
    }
